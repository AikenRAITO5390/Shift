...
//	    ResultSet rSet = null;
	...
	// 社員のみ取得
		...
		// バイトのみ取得
		public List<Worker> filter_b_w(Worker worker) throws Exception {
	        ...
	        try {
	            statement = connection.prepareStatement("select * from worker where store_id=? and worker_judge=false order by worker_id asc");
	            statement.setString(1, worker.getStoreId());

	            rSet = statement.executeQuery();
	            // Storeオブジェクトを作成して渡す
	            Store store = new Store();
	            store.setStoreId(worker.getStoreId());
		...
		// 社員のみ取得
			public List<Worker> filter_e_w(Worker worker) throws Exception {
